...
export function WatchDevices({onClose}:{onClose:()=>void}) {
 ...
 async function approve(e:FormEvent){e.preventDefault();await perform(async()=>{await watchApi('/pairings/approve','POST',{code});setCode('');setMessage('Watchを登録しました。Watchで通知を許可してください。')})}
 return <div className="dialog-backdrop"><section className="edit-dialog watch-devices" role="dialog" aria-modal="true" aria-labelledby="watch-title">
  ...
  <p>Watchに表示された8桁のコードを入力してください。</p>
  <form onSubmit={approve}><label>登録コード<input autoFocus inputMode="numeric" autoComplete="off" pattern="[0-9]{8}" maxLength={8} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,''))}/></label><button className="save-button" disabled={busy||code.length!==8}>登録する</button></form>
  {error&&<p role="alert">{error}</p>}{message&&<p role="status">{message}</p>}
  <h3>登録したWatch</h3>
  {devices.length===0?<p>登録済みのWatchはありません。</p>:<ul>{devices.map(d=><li key={d.id}><span>{new Date(d.created_at).toLocaleDateString()} 登録<br/>{d.notifications_enabled?'通知登録済み':'Watchで通知設定を確認してください'}</span><button className="text-button" disabled={busy} onClick={()=>perform(()=>watchApi(`/devices/${d.id}`,'DELETE'))}>解除する</button></li>)}</ul>}
 </section></div>
}
