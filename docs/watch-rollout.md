...
実行中のThreadを再タップすると継続確認し、別のThreadなら切り替える。
アプリ画面の独立した「続けている」ボタンと、現在のThreadの重複表示は削除した。
通知の「続けている」は維持する。55分から60分の間はアプリにも確認を促す文章を出す。
各行に実行状態の丸と委任先アイコンを置き、実行中のオレンジとAgentのロボットを同時に表示できる。
人物は他の人への委任、月は未委任を示す。VoiceOverにも状態とタップ操作を伝える。
